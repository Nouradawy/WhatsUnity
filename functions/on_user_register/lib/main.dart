...
import 'package:dart_appwrite/dart_appwrite.dart' show AppwriteException, Client, Databases, ID, Query;
...
Client _buildClient(Map<String, String> headers) {
  final endpoint = _env('APPWRITE_FUNCTION_ENDPOINT') ?? _env('APPWRITE_ENDPOINT') ?? 'https://cloud.appwrite.io/v1';
  var project = _env('APPWRITE_FUNCTION_PROJECT_ID') ?? _env('APPWRITE_PROJECT_ID');
  project ??= headers['x-appwrite-project'];
  if (project == null || project.isEmpty) {
    throw StateError('Missing project id: set APPWRITE_FUNCTION_PROJECT_ID or x-appwrite-project');
  }
...
String _requireDatabaseId() {
  final id = _env('APPWRITE_DATABASE_ID') ?? _env('APPWRITE_FUNCTION_DATABASE_ID');
  if (id == null || id.isEmpty) {
    throw StateError('Set APPWRITE_DATABASE_ID for this function (e.g. same as Flutter .env).');
  }
  return id;
}
...
Future<dynamic> main(final context) async {
  ...
  Map<String, Object?> err(String message, [Object? st]) {
    return {'ok': false, 'error': message, if (st != null) 'details': st.toString()};
  }
  ...
  try {
    ...
    // ---- Profile
    try {
      final display = (fullName != null && fullName.isNotEmpty) ? fullName : name;
