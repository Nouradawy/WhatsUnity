...
| profile | relationship | — | yes | — | no | **Profiles** / many-to-one / **Cascade** |
| user | relationship | — | yes | — | no | **Users** / one-to-one / **Cascade** |
...
| deleted_at | datetime | — | no | null | no |  |
| profile | relationship | — | yes | — | no | **Profiles** / many-to-one / **Cascade** |
| user | relationship | — | yes | — | no | **Users** / many-to-one / **Cascade** |
...
### 2.17 `gate_passes` (new)

Visitor gate passes generated by residents for security scan/check at compound gates.

| attribute | type | size | required | default | array? | notes |
| --- | --- | --- | --- | --- | --- | --- |
| compound_id | string | 36 | yes | — | no |  |
| resident_id | string | 36 | yes | — | no | profile/account id |
| guest_name | string | 256 | yes | — | no |  |
| purpose | string | 512 | yes | — | no | e.g. delivery, visit |
| valid_until | datetime | — | yes | — | no | pass expiry |
| status | string | 32 | yes | active | no | active/revoked/expired |
| qr_payload | string | 10000 | yes | — | no | signed/encoded QR payload string |
| version | integer | — | yes | 0 | no | LWW |
| deleted_at | datetime | — | no | null | no | soft delete |

**Indexes**

| key | type | attributes | order |
| --- | --- | --- | --- |
| idx_gate_passes_compound_resident | key | compound_id, resident_id | asc, asc |
| idx_gate_passes_valid_until | key | valid_until | asc |
| idx_gate_passes_status | key | status | asc |

---

### 2.18 `notification_preferences` (new)

Stores user-specific notification toggle preferences for each channel type. Used by server-side push functions to honor client-side "mute" settings.

| attribute | type | size | required | default | array? | notes |
| --- | --- | --- | --- | --- | --- | --- |
| user_id | string | 36 | yes | — | no | document $id = user_id |
| general_chat_enabled | boolean | — | yes | true | no |  |
| building_chat_enabled | boolean | — | yes | true | no |  |
| admin_notifications_enabled | boolean | — | yes | true | no |  |
| maintenance_notifications_enabled | boolean | — | yes | true | no |  |
| version | integer | — | yes | 0 | no | LWW |
| profile | relationship | — | yes | — | no | **Profiles** / many-to-one / **Cascade** |
...
| key | type | attributes | order |
| --- | --- | --- | --- |
| idx_notif_pref_user | unique | user_id | asc |
