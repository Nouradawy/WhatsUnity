...
import 'package:flutter/foundation.dart' show ValueListenable, ValueNotifier, kIsWeb, kDebugMode;
...
class _GeneralChatState extends State<GeneralChat>
  ...
  Future<void> _applyMessagesToController(
    ...
    try {
      ...
      AppLogger.e(
          "_applyMessagesToController: setMessages threw — recovering with Chat widget recreation",
          tag: 'GeneralChat',
          error: e,
          stackTrace: st);
  ...
  Future<void> _initializeChat() async {
    ...
    if (kDebugMode) {
      AppLogger.d("Building number: $buildingNumber", tag: 'GeneralChat');
    }
      ...
      try {
        ...
        AppLogger.e("_initializeChat network resolve failed", tag: 'GeneralChat', error: error, stackTrace: st);
      }
      ...
      if (resolvedChannelId == null || resolvedChannelId.isEmpty) {
        ...
        if (resolvedChannelId != null) {
          AppLogger.d("Using cached channel id (offline or empty resolve): ${widget.channelName} → $resolvedChannelId", tag: 'GeneralChat');
        }
      }
      ...
      if (_channelId != null) {
        ...
        AppLogger.w("Channel ID not found for compound ${widget.compoundId}", tag: 'GeneralChat');
      }
    } catch (error, st) {
      AppLogger.e("_initializeChat error", tag: 'GeneralChat', error: error, stackTrace: st);
    } finally {
      if (mounted) setState(() => _isInitializing = false);
    }
  }
  ...
  Future<void> _waitForChatMemberToLoad(
    ...
    while (mounted) {
      ...
      if (DateTime.now().isAfter(deadline)) {
        AppLogger.w("Timed out waiting for ChatMember; proceeding without it.", tag: 'GeneralChat');
        return;
      }
  ...
  Future<void> _handleImageSelection() async {
    ...
    try {
      ...
      AppLogger.e("Image upload failed", tag: 'GeneralChat', error: e, stackTrace: st);
      imageUrl = null;
    }
  ...
  Future<void> _createPollMessage(
    ...
    if (_channelId == null) return;
    final chatCubit = context.read<ChatCubit>();
    await chatCubit.sendPollMessage(
      text: question,
  ...
  Future<void> _resolveUserById(String id) async {
    ...
    try {
      ...
      AppLogger.e("Error resolving user $id", tag: 'GeneralChat', error: e, stackTrace: st);
    } finally {
  ...
  void _startPollingForAudioMessage(types.AudioMessage message) {
    ...
    Future<void> pollTick() async {
      ...
      try {
        ...
        AppLogger.e("audio ready metadata update failed", tag: 'GeneralChat', error: e, stackTrace: st);
      }
    }
